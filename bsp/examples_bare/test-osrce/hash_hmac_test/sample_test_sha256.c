...
u32 sample_test_sha256(void)
{
    ...
    node[2].msg_bytes = 32;
    ret = sha256_node_steps(node, 3, digest);
    if(HASH_SUCCESS != ret)
    {
        return 1;
    }
    ...
    if(memcmp_(std_digest, digest, digest_byte_len))
    {
        printf("\r\n SHA256 sample test 5 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA256 sample test 5 success");//
    }
    ...
    node[2].msg_addr  = msg +48;
    node[2].msg_bytes = 32;
    ret = hash_node_steps(HASH_SHA256, node, 3, digest);
    if(HASH_SUCCESS != ret)
    {
        return 1;
    }
    ...
    if(memcmp_(std_digest, digest, digest_byte_len))
    {
        printf("\r\n SHA256 sample test 6 error");
        return 1;
    }
    else
    {
        printf("\r\n SHA256 sample test 6 success");
    }
#endif
...
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = sha256_dma_update_blocks(sha256_dma_ctx, addr_h_0, (u32)msg_buf, blocks_bytes);
#else
    ret = sha256_dma_update_blocks(sha256_dma_ctx, msg_buf, blocks_bytes);
#endif
    if(HASH_SUCCESS != ret)
    {
        return 1;
...
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = sha256_dma_final(sha256_dma_ctx, addr_h_0, ((u32)msg_buf)+blocks_bytes, remainder_bytes, addr_h_0, (u32)digest_buf);
#else
    ret = sha256_dma_final(sha256_dma_ctx, msg_buf+blocks_bytes/4, remainder_bytes, digest_buf);
#endif
    if(HASH_SUCCESS != ret)
    {
        return 1;
    ...
    uint32_clear(digest_buf, digest_byte_len/4);
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = sha256_dma(addr_h_0, (u32)msg_buf, msg_byte_len, addr_h_0, (u32)digest_buf, hash_call_manage);
#else
    ...
    ret = sha256_dma_node_steps(dma_node, 2, addr_h_0, (u32)digest_buf, hash_call_manage);
#else
    dma_node[0].msg_addr  = msg_buf;
    dma_node[0].msg_bytes = blocks_bytes;
    dma_node[1].msg_addr  = dma_node[0].msg_addr + dma_node[0].msg_bytes/4;
    dma_node[1].msg_bytes = remainder_bytes;
    ret = sha256_dma_node_steps(dma_node, 2, digest_buf, hash_call_manage);
#endif
    ...
    uint32_clear(digest_buf, digest_byte_len/4);
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = hash_dma(HASH_SHA256, addr_h_0, (u32)msg_buf, msg_byte_len, addr_h_0, (u32)digest_buf, hash_call_manage);
#else
    ...
    if(uint32_BigNumCmp((u32 *)std_digest, digest_byte_len/4, digest_buf, digest_byte_len/4))
    {
        printf("\r\n SHA256 dma sample test 5 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA256 dma sample test 5 success");//
    }
    ...
    dma_node[1].msg_addr_l  = dma_node[0].msg_addr_l + dma_node[0].msg_bytes;
    dma_node[1].msg_bytes   = remainder_bytes;
    ret = hash_dma_node_steps(HASH_SHA256, dma_node, 2, addr_h_0, (u32)digest_buf, hash_call_manage);
#else
    dma_node[0].msg_addr  = msg_buf;
    dma_node[0].msg_bytes = blocks_bytes;
    dma_node[1].msg_addr  = dma_node[0].msg_addr + dma_node[0].msg_bytes/4;
    dma_node[1].msg_bytes = remainder_bytes;
    ret = hash_dma_node_steps(HASH_SHA256, dma_node, 2, digest_buf, hash_call_manage);
#endif
    ...
    if(uint32_BigNumCmp((u32 *)std_digest, digest_byte_len/4, digest_buf, digest_byte_len/4))
    {
        printf("\r\n SHA256 dma sample test 6 error");
        return 1;
    }
    else
    {
        printf("\r\n SHA256 dma sample test 6 success");
    }
