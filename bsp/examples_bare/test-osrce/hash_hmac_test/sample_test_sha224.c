...
u32 sample_test_sha224(void)
{
    ...
    {
        printf("\r\n SHA224 sample test 1 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA224 sample test 1 success");//
    }
    ...
    if(memcmp_(std_digest, digest, digest_byte_len))
    {
        printf("\r\n SHA224 sample test 2 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA224 sample test 2 success");//
    }
    ...
    if(memcmp_(std_digest, digest, digest_byte_len))
    {
        printf("\r\n SHA224 sample test 4 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA224 sample test 4 success");//
    }
    ...
    if(memcmp_(std_digest, digest, digest_byte_len))
    {
        printf("\r\n SHA224 sample test 5 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA224 sample test 5 success");//
    }
...
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = sha224_dma_update_blocks(sha224_dma_ctx, addr_h_0, (u32)msg_buf, blocks_bytes);
#else
...
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = sha224_dma_final(sha224_dma_ctx, addr_h_0, ((u32)msg_buf)+blocks_bytes, remainder_bytes, addr_h_0, (u32)digest_buf);
#else
    ...
    {
        printf("\r\n SHA224 dma sample test 1 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA224 dma sample test 1 success");//
    }

    /**************** dma test 2 ****************/
    uint32_clear(digest_buf, digest_byte_len/4);
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = sha224_dma(addr_h_0, (u32)msg_buf, msg_byte_len, addr_h_0, (u32)digest_buf, hash_call_manage);
#else
    ...
    if(uint32_BigNumCmp((u32 *)std_digest, digest_byte_len/4, digest_buf, digest_byte_len/4))
    {
        printf("\r\n SHA224 dma sample test 2 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA224 dma sample test 2 success");//
    }
    ...
    ret = sha224_dma_node_steps(dma_node, 2, addr_h_0, (u32)digest_buf, hash_call_manage);
#else
    ...
    if(uint32_BigNumCmp((u32 *)std_digest, digest_byte_len/4, digest_buf, digest_byte_len/4))
    {
        printf("\r\n SHA224 dma sample test 4 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA224 dma sample test 4 success");//
    }

    /**************** dma test 5 ****************/
    uint32_clear(digest_buf, digest_byte_len/4);
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = hash_dma(HASH_SHA224, addr_h_0, (u32)msg_buf, msg_byte_len, addr_h_0, (u32)digest_buf, hash_call_manage);
#else
    ...
    if(uint32_BigNumCmp((u32 *)std_digest, digest_byte_len/4, digest_buf, digest_byte_len/4))
    {
        printf("\r\n SHA224 dma sample test 5 error");//
        return 1;
    }
    else
    {
        printf("\r\n SHA224 dma sample test 5 success");//
    }
    ...
    dma_node[1].msg_addr_l  = dma_node[0].msg_addr_l + dma_node[0].msg_bytes;
    dma_node[1].msg_bytes   = remainder_bytes;
    ret = hash_dma_node_steps(HASH_SHA224, dma_node, 2, addr_h_0, (u32)digest_buf, hash_call_manage);
#else
    dma_node[0].msg_addr  = msg_buf;
    dma_node[0].msg_bytes = blocks_bytes;
    dma_node[1].msg_addr  = dma_node[0].msg_addr + dma_node[0].msg_bytes/4;
