...
static zx_button_config_t* g_button_config = NULL;

static void __zx_button_update_task(void *arg);
static struct rt_thread __zx_button_thread;

static ALIGN(8) rt_uint8_t __zx_button_thread_stack[1024 * 8];    // 8K

void zx_button_init(zx_button_config_t* config) {
	g_button_config = (zx_button_config_t *)calloc(1, sizeof(zx_button_config_t));
	memcpy(g_button_config, config, sizeof(zx_button_config_t));
	if (g_button_config->update_task.en) {
    	rt_thread_init(&__zx_button_thread, "btn-update", __zx_button_update_task, RT_NULL,
           &__zx_button_thread_stack[0], sizeof(__zx_button_thread_stack), g_button_config->update_task.priority, 0);
    	rt_thread_startup(&__zx_button_thread);
    }
}
...
press_event_t zx_button_get_event(btn_handle_t btn_handle)
{
...
int zx_button_start(btn_handle_t btn_handle)
{
	btn_data_t* handle = (btn_data_t*)btn_handle;
...
void zx_button_stop(btn_handle_t btn_handle)
{
	btn_data_t* handle = (btn_data_t*)btn_handle;
	btn_data_t** curr;
...
static void __zx_button_update_task(void *arg)
{
	for (;;) {
		zx_button_update();
		rt_thread_mdelay(g_button_config->ticks_interval_ms);
	}
}
