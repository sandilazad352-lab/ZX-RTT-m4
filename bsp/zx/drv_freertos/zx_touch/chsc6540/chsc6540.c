...
static ZX_t chsc6540_init(const touch_panel_config_t *config) {
    if (config->rst_pin > -1) {
        gpio_set_func_as_gpio(config->rst_pin);
...
static ZX_t chsc6540_deinit(void) {
    return ZX_OK;
}

static ZX_t chsc6540_read_point_data(touch_panel_points_t *point) {
    ZX_PARAM_CHECK(point != NULL);
    // Maybe it`s not correct, some driver x pos 1, 2, y pos: 3, 4 
    uint8_t data[11] = {0x0};
    if (i2c_read_bytes(chsc6540_device, 0x02, data, 11) != I2C_OK) {
        vTaskDelay(pdMS_TO_TICKS(10));
