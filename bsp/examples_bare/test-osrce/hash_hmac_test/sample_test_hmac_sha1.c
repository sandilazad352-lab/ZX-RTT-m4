...
u32 sample_test_hmac_sha1(void)
{
    ...
    u8 msg_byte_len = 80;
    u8 key_byte_len = 16;
    u8 mac_byte_len = 20;  //digest_byte_len
    ...
    {
        printf("\r\n HMAC-SHA1 sample test 1 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 sample test 1 success");
    }
    ...
    if(memcmp_(std_mac, mac, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 sample test 2 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 sample test 2 success");
    }
    ...
    node[2].msg_bytes = 32;
    ret = hmac_sha1_node_steps(key, 0, key_byte_len, node, 3, mac);
    if(HASH_SUCCESS != ret)
    {
        return 1;
    }
    ...
    if(memcmp_(std_mac, mac, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 sample test 3 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 sample test 3 success");
    }
#endif
    ...
    if(memcmp_(std_mac, mac, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 sample test 4 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 sample test 4 success");
    }
    ...
    if(memcmp_(std_mac, mac, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 sample test 5 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 sample test 5 success");
    }
    ...
    node[2].msg_addr  = msg +48;
    node[2].msg_bytes = 32;
    ret = hmac_node_steps(HASH_SHA1, key, 0, key_byte_len, node, 3, mac);
    if(HASH_SUCCESS != ret)
    {
        return 1;
    }
    ...
    if(memcmp_(std_mac, mac, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 sample test 6 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 sample test 6 success");
    }
#endif
...
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = hmac_sha1_dma_update_blocks(hmac_sha1_dma_ctx, addr_h_0, (u32)msg_buf, blocks_bytes);
#else
    ret = hmac_sha1_dma_update_blocks(hmac_sha1_dma_ctx, msg_buf, blocks_bytes);
#endif
    if(HASH_SUCCESS != ret)
    {
        return 1;
...
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = hmac_sha1_dma_final(hmac_sha1_dma_ctx, addr_h_0, ((u32)msg_buf)+blocks_bytes, remainder_bytes, addr_h_0, (u32)mac_buf);
#else
    ret = hmac_sha1_dma_final(hmac_sha1_dma_ctx, msg_buf+blocks_bytes/4, remainder_bytes, mac_buf);
#endif
    if(HASH_SUCCESS != ret)
    {
        return 1;
    ...
    {
        printf("\r\n HMAC-SHA1 dma sample test 1 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 dma sample test 1 success");
    }

    /**************** dma test 2 ****************/
    uint32_clear(mac_buf, mac_byte_len/4);
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = hmac_sha1_dma(key, 0, key_byte_len, addr_h_0, (u32)msg_buf, msg_byte_len, addr_h_0, (u32)mac_buf, hash_call_manage);
#else
    ...
    if(memcmp_(std_mac, (u8 *)mac_buf, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 dma sample test 2 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 dma sample test 2 success");
    }
    ...
    ret = hmac_sha1_dma_node_steps(key, 0, key_byte_len, dma_node, 2, addr_h_0, (u32)mac_buf, hash_call_manage);
#else
    dma_node[0].msg_addr  = msg_buf;
    dma_node[0].msg_bytes = blocks_bytes;
    dma_node[1].msg_addr  = dma_node[0].msg_addr + dma_node[0].msg_bytes/4;
    dma_node[1].msg_bytes = remainder_bytes;
    ret = hmac_sha1_dma_node_steps(key, 0, key_byte_len, dma_node, 2, mac_buf, hash_call_manage);
#endif
    ...
    if(memcmp_(std_mac, (u8 *)mac_buf, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 dma sample test 3 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 dma sample test 3 success");
    }
    ...
    if(memcmp_(std_mac, (u8 *)mac_buf, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 dma sample test 4 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 dma sample test 4 success");
    }

    /**************** dma test 5 ****************/
    uint32_clear(mac_buf, mac_byte_len/4);
#ifdef AIC_HASH_ADDRESS_HIGH_LOW
    ret = hmac_dma(HASH_SHA1, key, 0, key_byte_len, addr_h_0, (u32)msg_buf, msg_byte_len, addr_h_0, (u32)mac_buf, hash_call_manage);
#else
    ...
    if(memcmp_(std_mac, (u8 *)mac_buf, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 dma sample test 5 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 dma sample test 5 success");
    }
    ...
    dma_node[1].msg_addr_l  = dma_node[0].msg_addr_l + dma_node[0].msg_bytes;
    dma_node[1].msg_bytes   = remainder_bytes;
    ret = hmac_dma_node_steps(HASH_SHA1, key, 0, key_byte_len, dma_node, 2, addr_h_0, (u32)mac_buf, hash_call_manage);
#else
    dma_node[0].msg_addr  = msg_buf;
    dma_node[0].msg_bytes = blocks_bytes;
    dma_node[1].msg_addr  = dma_node[0].msg_addr + dma_node[0].msg_bytes/4;
    dma_node[1].msg_bytes = remainder_bytes;
    ret = hmac_dma_node_steps(HASH_SHA1, key, 0, key_byte_len, dma_node, 2, mac_buf, hash_call_manage);
#endif
    ...
    if(memcmp_(std_mac, (u8 *)mac_buf, mac_byte_len))
    {
        printf("\r\n HMAC-SHA1 dma sample test 6 error");
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA1 dma sample test 6 success");
    }
