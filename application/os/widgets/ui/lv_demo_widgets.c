...
static lv_timer_t *widgets_timer;
...
{
    lv_anim_t a;
    lv_coord_t y = lv_obj_get_content_height(parent) + lv_obj_get_style_pad_top(parent, LV_PART_MAIN) + lv_obj_get_style_pad_bottom(parent, LV_PART_MAIN) + lv_obj_get_style_border_width(parent, LV_PART_MAIN);

    lv_anim_del(NULL, (lv_anim_exec_xcb_t)lv_obj_set_scroll_y);
    lv_anim_init(&a);
    lv_anim_set_var(&a, parent);
    lv_anim_set_exec_cb(&a, (lv_anim_exec_xcb_t)lv_obj_set_scroll_y);
...
static void tab_changer_task_cb(lv_timer_t * timer)
{
...
static void analytics_create(lv_obj_t * parent)
{
    ...
    widgets_timer = lv_timer_create(meter2_timer_cb, 100, meter2_indic);
