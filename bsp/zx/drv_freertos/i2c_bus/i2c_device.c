...
I2CDevice_t i2c_malloc_device(int i2c_num, int8_t sda, int8_t scl, uint32_t freq, uint8_t device_addr) {
    ...
    i2c_log_i("New device malloc, scl: %d, sda: %d, freq: %"PRIi32" HZ",
...
exit:
    if (port_new != I2C_PORT_NO_INIT) {
        ...
        i2c_log_e("I2C config apply failed, scl: %d, sda: %d, freq: %"PRIi32" HZ", select_port->scl, select_port->sda, select_port->freq);
        return I2C_FAIL;
    }
    i2c_port_used[select_port->i2c_num] = select_port;
    i2c_log_i("I2C config update, scl: %d, sda: %d, freq: %"PRIi32" HZ", select_port->scl, select_port->sda, select_port->freq);
    return I2C_OK;
}
