...
static void send_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        // rt_kprintf("send_data[0]:%d\n", data[0]);
        // rt_kprintf("send_data[1]:%d\n", data[1]);
        ...
        switch (error_code) {
        case MB_MRE_NO_ERR:
            rt_kprintf("MB_MRE_NO_ERR\n");
        break;
        case MB_MRE_NO_REG:
            rt_kprintf("MB_MRE_NO_REG\n");
        break;
        case MB_MRE_ILL_ARG:
            rt_kprintf("MB_MRE_ILL_ARG\n");
        break;
        case MB_MRE_REV_DATA:
            rt_kprintf("MB_MRE_REV_DATA\n");
        break;
        case MB_MRE_TIMEDOUT:
            rt_kprintf("MB_MRE_TIMEDOUT\n");
        break;
        default:
            rt_kprintf("default\n");
        break;
...
static int mb_master_sample(int argc, char **argv)
{
    ...
    tid1 = rt_thread_create("md_m_poll", mb_master_poll, RT_NULL, 1024*3, MB_POLL_THREAD_PRIORITY, 10);
    if (tid1 != RT_NULL)
    {
    ...
    tid2 = rt_thread_create("md_m_send", send_thread_entry, RT_NULL, 1024*3, MB_SEND_THREAD_PRIORITY - 2, 10);
    if (tid2 != RT_NULL)
    {
