...
static int set_ck_cfg(struct mpp_fb * fd, int enable, int val)
{
    int ret = 0;
    ...
    g = val >> 8 & 0xff;
    b = val & 0xff;
    val = (r | r >> 5) << 16 | (g | g >> 6) << 8 | (b | b >> 5); 
#endif

    ck.layer_id = 1;
    ck.enable = enable;
    ck.value = val;
    ret = mpp_fb_ioctl(fd_dev, AICFB_UPDATE_CK_CONFIG, &ck);
    if (ret < 0)
...
static void main_tapview_event(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED) {
        ...
        if (tab_num == 0) {
            ...
            lv_obj_set_style_bg_color(lv_scr_act(), lv_color_make(0x80, 0x80, 0x80), 0);
...
#ifdef VIDEO_PLAYER
            set_ck_cfg(fd_dev, 1, 0x00808080);
#endif
        }
    }
}
