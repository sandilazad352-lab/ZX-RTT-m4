...
void label_key_event_handler(lv_event_t * e)
{
    ...
    if ((code == LV_EVENT_SHORT_CLICKED) || (code == LV_EVENT_LONG_PRESSED))
    {
        ...
        if(code == LV_EVENT_SHORT_CLICKED)
            xReceivedEvent.lDataArray[1] = E_BUTTON_SINGLE_CLICK;
        else if(code == LV_EVENT_LONG_PRESSED)
            xReceivedEvent.lDataArray[1] = E_BUTTON_LONG_PRESS_START;
...
{
    // LV_IMG_DECLARE(mask00);
    // lv_obj_t *logo = lv_img_create(lv_scr_act());
    // lv_img_set_src(logo, &mask00);
    // lv_obj_center(logo);
