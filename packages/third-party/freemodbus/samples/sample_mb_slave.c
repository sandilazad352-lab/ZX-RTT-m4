...
static int mb_slave_sample(int argc, char **argv)
{
    ...
    tid1 = rt_thread_create("md_s_poll", mb_slave_poll, argv[1], 1024*2, MB_POLL_THREAD_PRIORITY, 10);
    if (tid1 != RT_NULL)
    {
    ...
    tid2 = rt_thread_create("md_s_send", send_thread_entry, RT_NULL, 1024*2, MB_SEND_THREAD_PRIORITY, 10);
    if (tid2 != RT_NULL)
    {
