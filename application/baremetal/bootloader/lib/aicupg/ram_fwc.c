...
static struct zx_image_info image_info;
...
static enum upg_dev_type get_device_type(struct zx_image_info *info)
{
...
s32 ram_fwc_data_write(struct fwc_info *fwc, u8 *buf, s32 len)
{
    ...
    if (len >= sizeof(*info)) {
        info = (struct zx_image_info *)fwc->priv;
