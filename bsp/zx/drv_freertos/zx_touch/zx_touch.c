...
static void touch_calibration_points(touch_panel_points_t *point) {
    ...
    if (point->curx[0] >= g_panel_config->width) {
        if (point->event == TOUCH_EVT_PRESS) {
            printf("Touch width %d exceed the maximum value %d", point->curx[0], g_panel_config->width);
        }
    ...
    if (point->cury[0] >= g_panel_config->height) {
        if (point->event == TOUCH_EVT_PRESS) {
            printf("Touch height %d exceed the maximum value %d", point->cury[0], g_panel_config->height);
        }
...
ZX_t touch_init(touch_panel_driver_t* touch_panel, touch_panel_config_t* panel_config) {
    ZX_PARAM_CHECK(touch_panel != NULL);
    ZX_PARAM_CHECK(panel_config != NULL);
    g_touch_panel = touch_panel;
    g_panel_config = (touch_panel_config_t*)ZX_MALLOC(sizeof(touch_panel_config_t));
    ZX_PARAM_CHECK(g_panel_config != NULL);
    memcpy(g_panel_config, panel_config, sizeof(touch_panel_config_t));
    if (panel_config->intr_pin > -1) {
        gpio_set_func_as_gpio(panel_config->intr_pin);
        gpio_set_direction(panel_config->intr_pin, GPIO_MODE_INPUT);
    }
    if (panel_config->task_en) {
        TaskHandle_t task_handle;
        xTaskCreate(touch_read_task, "touch", panel_config->task_stack_size, (void *)(panel_config->intr_pin < 0), panel_config->task_priority, &task_handle);
        if (panel_config->intr_pin > -1) {
            gpio_irq_attach(panel_config->intr_pin, GPIO_IRQ_MODE_FALLING_EDGE, touch_isr_handler, task_handle);
            gpio_irq_enable(panel_config->intr_pin, 1);
        }
    } else {
        g_touch_panel->init(g_panel_config);
    }
...
ZX_t touch_read_points(touch_panel_points_t *point) {
    ZX_PARAM_CHECK(g_touch_panel != NULL);
    ZX_PARAM_CHECK(point != NULL);
    point->point_num = 1;
